...
def model_DNN(df, convert=False):
    if convert:
    ...
    pipe = make_pipeline(Imputer(strategy="median", axis=0, copy=False), 
                         StandardScaler())
    ...
    callback_list = [keras.callbacks.EarlyStopping(monitor='val_loss', patience=1, verbose=1),]
    model.fit(X_train, y, epochs=5, batch_size=1024)
    y_pred = model.predict_proba(X_test)
    result['TARGET'] = y_pred
    result.to_csv("submission_DNN.csv", index=False)
